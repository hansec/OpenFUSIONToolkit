...
USE oft_gs, ONLY: gs_eq, gs_save_fields, gs_save_fgrid, gs_setup_walls, build_dels, &
  ...
  gs_coil_mutual, gs_coil_mutual_distributed, gs_get_cond_weights, gs_set_cond_weights
...
!> Needs docs
!------------------------------------------------------------------------------
SUBROUTINE tokamaker_recon_run(tMaker_ptr,vacuum,settings,coil_wt,nonax_corr,error_flag) BIND(C,NAME="tokamaker_recon_run")
TYPE(c_ptr), VALUE, INTENT(in) :: tMaker_ptr !< Pointer to TokaMaker object
...
IF(c_associated(nonax_corr))THEN
  CALL c_f_pointer(nonax_corr, nonax_corr_tmp, [tMaker_obj%gs%ncond_eigs+1,100])
ELSE
  NULLIFY(nonax_corr_tmp)
END IF
IF(c_associated(coil_wt))THEN
  CALL c_f_pointer(coil_wt, coil_wt_tmp, [tMaker_obj%gs%ncoils])
  CALL fit_gs(tMaker_obj%gs,infile,outfile,fitI=fitI,fitP=fitP,fitPnorm=fitPnorm,&
...
! !> Needs docs
! !------------------------------------------------------------------------------
! SUBROUTINE tokamaker_recon_set_nonax(tMaker_ptr,icon,nonax_corr,error_str) BIND(C,NAME="tokamaker_recon_set_nonax")
! TYPE(c_ptr), VALUE, INTENT(in) :: tMaker_ptr !< Pointer to TokaMaker object
...
! TYPE(tokamaker_instance), POINTER :: tMaker_obj
! IF(.NOT.tokamaker_ccast(tMaker_ptr,tMaker_obj,error_str))RETURN
! CALL c_f_pointer(nonax_corr, vals_tmp, [tMaker_obj%gs%ncoils])
...
!> Needs docs
!------------------------------------------------------------------------------
SUBROUTINE tokamaker_set_cond_eigs(tMaker_ptr,neigs,cond_eigs,error_str) BIND(C,NAME="tokamaker_set_cond_eigs")
TYPE(c_ptr), VALUE, INTENT(in) :: tMaker_ptr !< TokaMaker instance
...
IF(.NOT.tokamaker_ccast(tMaker_ptr,tMaker_obj,error_str))RETURN
!
IF(ASSOCIATED(tMaker_obj%gs%cond_weights))DEALLOCATE(tMaker_obj%gs%cond_weights)
!
CALL c_f_pointer(neigs, neigs_tmp, [tMaker_obj%gs%ncond_regs])
tMaker_obj%gs%ncond_eigs=SUM(neigs_tmp)
CALL c_f_pointer(cond_eigs, eigs_tmp, [tMaker_obj%gs%psi%n,tMaker_obj%gs%ncond_eigs])
tMaker_obj%gs%ncond_eigs=0
DO i=1,tMaker_obj%gs%ncond_regs
  IF(ASSOCIATED(tMaker_obj%gs%cond_regions(i)%weights))DEALLOCATE(tMaker_obj%gs%cond_regions(i)%weights)
  IF(ASSOCIATED(tMaker_obj%gs%cond_regions(i)%fixed))DEALLOCATE(tMaker_obj%gs%cond_regions(i)%fixed)
  IF(ASSOCIATED(tMaker_obj%gs%cond_regions(i)%mind))DEALLOCATE(tMaker_obj%gs%cond_regions(i)%mind)
  IF(ASSOCIATED(tMaker_obj%gs%cond_regions(i)%mtype))DEALLOCATE(tMaker_obj%gs%cond_regions(i)%mtype)
  IF(ASSOCIATED(tMaker_obj%gs%cond_regions(i)%eig_map))DEALLOCATE(tMaker_obj%gs%cond_regions(i)%eig_map)
  IF(ASSOCIATED(tMaker_obj%gs%cond_regions(i)%fit_scales))DEALLOCATE(tMaker_obj%gs%cond_regions(i)%fit_scales)
  IF(ASSOCIATED(tMaker_obj%gs%cond_regions(i)%psi_eig))THEN
    DO j=1,tMaker_obj%gs%cond_regions(i)%neigs
      CALL tMaker_obj%gs%cond_regions(i)%psi_eig(j)%f%delete
    END DO
    DEALLOCATE(tMaker_obj%gs%cond_regions(i)%psi_eig)
  END IF
  !---TODO: Need to deallocated conductor currents if specified
  !
  tMaker_obj%gs%cond_regions(i)%neigs=neigs_tmp(i)
  IF(tMaker_obj%gs%cond_regions(i)%neigs>0)THEN
    ALLOCATE(tMaker_obj%gs%cond_regions(i)%weights(tMaker_obj%gs%cond_regions(i)%neigs))
    ALLOCATE(tMaker_obj%gs%cond_regions(i)%fixed(tMaker_obj%gs%cond_regions(i)%neigs))
    ALLOCATE(tMaker_obj%gs%cond_regions(i)%mind(tMaker_obj%gs%cond_regions(i)%neigs))
    ALLOCATE(tMaker_obj%gs%cond_regions(i)%mtype(tMaker_obj%gs%cond_regions(i)%neigs))
    ALLOCATE(tMaker_obj%gs%cond_regions(i)%psi_eig(tMaker_obj%gs%cond_regions(i)%neigs))
    ALLOCATE(tMaker_obj%gs%cond_regions(i)%eig_map(tMaker_obj%gs%cond_regions(i)%neigs))
    ALLOCATE(tMaker_obj%gs%cond_regions(i)%fit_scales(tMaker_obj%gs%cond_regions(i)%neigs))
    tMaker_obj%gs%cond_regions(i)%weights=0.d0
    tMaker_obj%gs%cond_regions(i)%fixed=.FALSE.
    tMaker_obj%gs%cond_regions(i)%mind=1
    tMaker_obj%gs%cond_regions(i)%mtype=[(j,j=1,tMaker_obj%gs%cond_regions(i)%neigs)]
    tMaker_obj%gs%cond_regions(i)%eig_map=[(j,j=1,tMaker_obj%gs%cond_regions(i)%neigs)]+tMaker_obj%gs%ncond_eigs
    tMaker_obj%gs%cond_regions(i)%fit_scales=1.d0
    DO j=1,tMaker_obj%gs%cond_regions(i)%neigs
      tMaker_obj%gs%ncond_eigs=tMaker_obj%gs%ncond_eigs+1
      CALL tMaker_obj%gs%psi%new(tMaker_obj%gs%cond_regions(i)%psi_eig(j)%f)
      CALL tMaker_obj%gs%cond_regions(i)%psi_eig(j)%f%restore_local(eigs_tmp(:,tMaker_obj%gs%ncond_eigs))
    END DO
  END IF
END DO
ALLOCATE(tMaker_obj%gs%cond_weights(tMaker_obj%gs%ncond_eigs))
CALL gs_get_cond_weights(tMaker_obj%gs,tMaker_obj%gs%cond_weights,.FALSE.)
CALL gs_set_cond_weights(tMaker_obj%gs,tMaker_obj%gs%cond_weights,.FALSE.)
END SUBROUTINE tokamaker_set_cond_eigs
!------------------------------------------------------------------------------
!> Needs docs
!------------------------------------------------------------------------------
SUBROUTINE tokamaker_get_cond_weights(tMaker_ptr,weights,error_str) BIND(C,NAME="tokamaker_get_cond_weights")
TYPE(c_ptr), VALUE, INTENT(in) :: tMaker_ptr !< TokaMaker instance
...
TYPE(tokamaker_instance), POINTER :: tMaker_obj
IF(.NOT.tokamaker_ccast(tMaker_ptr,tMaker_obj,error_str))RETURN
IF(tMaker_obj%gs%ncond_eigs<=0)RETURN
CALL c_f_pointer(weights, vals_tmp, [tMaker_obj%gs%ncond_eigs])
vals_tmp=tMaker_obj%gs%cond_weights
