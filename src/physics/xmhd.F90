...
102 FORMAT(2X,A,ES11.3)
  ...
  WRITE(oft_ounit,100)'B-Adv      = ',xmhd_adv_b
  ...
  IF(xmhd_monitor_div)WRITE(oft_ounit,'(2X,A,2ES11.3)')'Div Error  = ',derror,jump_error
  WRITE(oft_ounit,102)'# B-DOFs   = ',REAL(oft_hcurl%global%ne+oft_hgrad%global%ne,8)
...
DO i=1,nsteps
    ...
    IF(MOD(rst_ind+i,nclean)==0)THEN
      CALL oft_xmhd_pop(u,sub_fields)
      IF(oft_env%head_proc)WRITE(oft_ounit,*)'  Cleaning Divergence'
...
!------------------------------------------------------------------------------
    ...
    IF(xmhd_mfnk.AND.xmhd_opcount/=0.AND.(nksolver%lits>baseit+1.OR.nksolver%nlits>=nl_update))THEN
       IF(oft_env%head_proc)WRITE(oft_ounit,*)'Update Jacobian',nksolver%lits,baseit,nredo
       IF(nredo<1)THEN
         jac_dt=dt/2.d0
...
102 FORMAT(2X,A,ES11.3)
  ...
  WRITE(oft_ounit,102)'Kin Energy = ',ver
  IF(xmhd_monitor_div)WRITE(oft_ounit,'(2X,A,2ES11.3)')'Div Error  = ',derror,jump_error
  WRITE(oft_ounit,102)'# B-DOFs   = ',REAL(oft_hcurl%global%ne+oft_hgrad%global%ne,4)
...
build_optime=0.d0
dot_time=0.d0
IF(oft_env%head_proc)WRITE(oft_ounit,'(A)')'Starting profiling'
comm_times=0
DO i=1,10
  IF(oft_env%head_proc)CALL mytimer%tick
  CALL xmhd_set_ops(u)
...
END DO
IF(oft_env%head_proc)THEN
  WRITE(oft_ounit,'(A,ES11.3)')'  Dot    = ',dot_time/REAL(10*10*100,8)
  WRITE(oft_ounit,'(A,ES11.3)')'  MatVec = ',matvec_time/REAL(10*10*10,8)
  WRITE(oft_ounit,'(A,ES11.3)')'  NL Op  = ',nlop_time/REAL(10*10,8)
  WRITE(oft_ounit,'(A,ES11.3)')'  Build  = ',matvec_time/REAL(10,8)
END IF
CALL oft_finalize()
