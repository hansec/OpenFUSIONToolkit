...
!$omp Te_weights_loc,res_loc,jac_mat,jac_det,T_i,T_e,dT_i,dT_e) reduction(+:diag_vals)
ALLOCATE(basis_vals(oft_blagrange%nce),basis_grads(oft_blagrange%nce,3))
ALLOCATE(Ti_weights_loc(oft_blagrange%nce),Te_weights_loc(oft_blagrange%nce))
ALLOCATE(cell_dofs(oft_blagrange%nce),res_loc(oft_blagrange%nce,2))
!$omp do schedule(static)
DO i=1,mesh%nc
...
!------------------------------------------------------------------------------
  DO m=1,quad%np
    ...
    ! basis_vals=self%full_rops(:,m,i,1)
    ! basis_grads=self%full_rops(:,m,i,2:4)
    !---Reconstruct values of solution fields
    T_i = 0.d0; dT_i = 0.d0; T_e = 0.d0; dT_e = 0.d0
    !$omp simd reduction(+:T_i,dT_i,T_e,dT_e)
    DO jr=1,oft_blagrange%nce
...
quad=>oft_blagrange%quad
!
ALLOCATE(self%full_rops(oft_blagrange%nce,quad%np,mesh%nc,4))
self%full_rops=0.d0
...
ALLOCATE(self%template_rops(oft_blagrange%nce,quad%np,4))
self%template_rops=0.d0
ALLOCATE(self%jac_mat(quad%np,mesh%nc,3,3))
self%jac_mat=0.d0
ALLOCATE(self%cell_trans(oft_blagrange%nce,oft_blagrange%nce,mesh%nc,1))
...
ALLOCATE(basis_vals(oft_blagrange%nce),basis_grads(3,oft_blagrange%nce))
ALLOCATE(cell_dofs(oft_blagrange%nce),mass_template(oft_blagrange%nce,oft_blagrange%nce,4))
!!$omp do schedule(static)
DO i=1,mesh%nc
  curved=cell_is_curved(mesh,i) ! Straight cell test
  ! call oft_blagrange%ncdofs(i,cell_dofs) ! Get global index of local DOFs
...
!------------------------------------------------------------------------------
  ...
  DO m=1,quad%np
    ...
    self%jac_mat(m,i,:,:)=TRANSPOSE(jac_mat(:,1:3))
    !---Evaluate value and gradients of basis functions at current point
    DO jr=1,oft_blagrange%nce ! Loop over degrees of freedom
      CALL oft_blag_eval(oft_blagrange,i,jr,quad%pts(:,m),self%full_rops(jr,m,i,1))
      CALL oft_blag_geval(oft_blagrange,i,jr,quad%pts(:,m),self%full_rops(jr,m,i,2:4),jac_mat)
    END DO
    IF(i==1)THEN
      DO jr=1,oft_blagrange%nce
        CALL oft_blag_eval(oft_blagrange,i,jr,quad%pts(:,m),self%template_rops(jr,m,1),cell_orient=.FALSE.)
        CALL oft_blag_geval(oft_blagrange,i,jr,quad%pts(:,m),self%template_rops(jr,m,2:4),jac_mat,cell_orient=.FALSE.)
      END DO
    END IF
    ...
    !   basis_grads(:,jr) = 0.d0
    !   DO k=1,3
    !     basis_grads(:,jr) = basis_grads(:,jr) + self%template_rops(jr,m,k+1)*jac_mat(:,k)
    !   END DO
    ! END DO
    DO jr=1,oft_blagrange%nce
    ...
    END DO
  END DO
  CALL lapack_matinv(oft_blagrange%nce,self%cell_trans(:,:,i,1),jc)
...
self%ML_fe_rep%nlevels=ML_oft_blagrange%nlevels
...
self%ML_fe_rep%ml_fields(1)%ml=>ML_oft_blagrange
self%ML_fe_rep%field_tags(1)='Ti'
self%ML_fe_rep%ml_fields(2)%ml=>ML_oft_blagrange
...
self%Ti_bc=>oft_blagrange%global%gbe
self%Te_bc=>oft_blagrange%global%gbe
