...
TYPE(oft_gs_eq), TARGET :: GS_field
...
!---------------------------------------------------------------------------
GS_field%eq_file='dipole_mug.h5'
CALL GS_field%setup(mg_mesh%mesh)
GS_field%interp_mode=1
!---Generate mass matrix
NULLIFY(mop) ! Ensure the matrix is unallocated (pointer is NULL)
...
CALL oft_hcurl_grad_project(xmhd_ML_hcurl_grad%current_level,GS_field,v)
